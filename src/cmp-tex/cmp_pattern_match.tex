...
\begin{description}
    \item[Merge into empty case tree]
        $$
        m(\varepsilon, t) := t
        $$

    \item [Merge a new constant] The new constant is not present in the tree
        $t_1$. Just add the tree with the new constant to the first tree and
        rename the pattern variable.
        %
        $$
        \begin{array}{l}
            m(
                [x \mid \vec c \vec t, \, \varepsilon \mid d],\,
                [x_2 \mid c_2 t_2 \mid \varepsilon])
            \\
            :=
            [ x \mid
                (\vec c c_2) (\vec t t_2[x/x_2]), \, \varepsilon
                \mid m(d, t_2[x/x_2])]
        \end{array}
        $$

    \item [Merge a new constructor] The new constructor is not present in the
        tree $t_1$. Just add the tree with the new constructor to the first tree
        and rename the pattern variable.
        %
        $$
        m(
            [x \mid \vec c \vec t, \, \vec\ell \vec u \mid ],\,
            [x_2 \mid \ell_2 t_2 \mid \varepsilon])
        :=
        [ x \mid
        \vec c \vec t,\,
        (\vec\ell \ell_2)(\vec u t_2[x/x_2]
        \mid m(d,t_2[x/x_2])]
        $$

    \item [Merge a present constant or present constructor] I.e. merge
        $[x_2 \mid c_2 t_2 \mid \varepsilon]$
        or
        $[x_2 \mid \ell_2 t_2 \varepsilon d]$
        into
        $[x \mid \vec c \vec t,\, \vec\ell \vec u \mid d]$.

        This is done by replacing the corresponding subtree $t$ in $\vec c \vec
        t$ or $\vec \ell \vec u$ by the merged tree $m(t, t_2[x/x_2]$.
\end{description}
