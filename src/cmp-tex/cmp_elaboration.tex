...
The elaborator works like an operating system. There are a set of tasks which
can do actions like elaborate a subterm, unify two types, ... Each of these task
is either ready or waiting for some conditions.

Conditions can be atomic like wait for a hole to be filled, wait for a
unification constraint to be resolved. The can be set of conditions as well.
Conditions must not overlap. Either they are disjoint or one is a subset of the
other. The subset relation establishes a partial order on conditions.

As soon as a condition is satisfied, the corresponding tasks can be unqueued and
executed. The order of the conditions have to be respected.
...
It might be necessary to generate holes for implicit arguments an put not only
...
\begin{enumerate}
    ...
    \item Wait for $f$: The type of $f$ has to be a function type. Otherwise the
        hole would not be filled. Unify the argument type with the type of $a$.
        Generate $f a$ and put it into the hole.
\end{enumerate}

Note: $f a$ might contain unfilled holes.
